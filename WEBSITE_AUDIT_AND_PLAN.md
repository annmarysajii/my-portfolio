...
- **About ↔ Contact merge:** #contact now shares About's red family, fixed regardless of the theme toggle (same as About). First pass had two real bugs, both fixed in a follow-up commit: the texture was using the same light-tile + multiply technique as About, but against a noticeably darker red the multiply math compressed the grain toward invisible — swapped to the site's own existing dark-tile + screen convention (already used for `[data-theme="dark"] body::before`), which is the correct technique for a dark base regardless of site theme. Separately, the email link's color (`var(--blue)`) turned out to sit at almost the exact same luminance as the new red background (~1:1 contrast — effectively invisible), because it was pulling the theme-reactive blue variable into a section that doesn't participate in the theme toggle; replaced with a dedicated fixed light sky-blue (`#B8D4FF`, ~4.7:1 contrast against the red) alongside the section's other fixed `--c-*` colors.
