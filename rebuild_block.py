for filename in ['project.html']:
    ...
    import re
    # We will slice everything from the start of the `if (id === 'gobunny' || ... ) {`
    # up to the final `else { gal.innerHTML = ...`
    
    start_str = "if (id === 'gobunny' || id === 'green-arrow' || id === 'nangele' || id === 'original-tracks' || id === 'wellbeing-planner' || id === 'ntu-fest') {"
    end_str = "} else {\n                  let htmlStr = `<div class=\"masonry-container\">`;"
    ...
    if start_str in html and end_str in html:
        ...
        new_block = start_str + """
                  ...
                  if (id === 'gobunny') {
                      ...
                      htmlStr += `<div class="cs-section"><h2 class="cs-heading" style="font-size:1.8rem; margin-bottom:1.5rem; border-bottom:1px solid var(--ink12); padding-bottom:0.5rem;">Event Banners & Collateral</h2><div class="masonry-container">`;
                      if (cover) htmlStr += renderMedia(cover);
        ...
        print("Successfully rebuilt custom layout block!")
    else:
