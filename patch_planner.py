...
new_block = """} else if (id === 'wellbeing-planner') {
                    ...
                    htmlStr += `<div class="cs-section">
                        <h2 class="cs-heading" style="font-size: 2rem; border-bottom: 2px solid var(--ink12); padding-bottom: 0.5rem;">Final Published Design</h2>
                        <div class="masonry-container">`;
                    if (finalImg) htmlStr += renderMedia(finalImg).replace('masonry-item', 'masonry-item full-width');
                    htmlStr += `</div></div>`;
                    
                    htmlStr += `<div class="cs-section">
                        <h2 class="cs-heading" style="font-size: 1.5rem; margin-top: 2rem;">Ideation & Concept Drafts</h2>
                        <div class="masonry-container">`;
                    drafts.forEach(f => { htmlStr += renderMedia(f); });
                    htmlStr += `</div></div>`;
                } else if (id === 'original-tracks') {"""

if "id === 'wellbeing-planner'" not in html:
    html = html.replace(old_block, new_block)

with open('project.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Added campus planner custom layout")
