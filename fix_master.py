...
# 3. Custom injection logic block
# Replace the ENTIRE `if (id === 'gobunny' || ...)` block with a new controlled block!
# First we find the block
pattern = r"if \(id === 'gobunny' \|\| id === 'green-arrow' \|\| id === 'nangele'\) \{.*?gal\.innerHTML = htmlStr;\n\s*return;\n\s*\}"
# Wait, no. Let's just find the exact string boundaries.
start_str = "if (id === 'gobunny' || id === 'green-arrow' || id === 'nangele') {"
end_str = "gal.innerHTML = htmlStr;\n                return;\n            }"
if start_str in html and end_str in html:
    start_idx = html.find(start_str)
    end_idx = html.find(end_str) + len(end_str)
    old_block = html[start_idx:end_idx]
    
    # We will rewrite the ENTIRE custom intercept block securely!
    new_block = """if (id === 'gobunny' || id === 'green-arrow' || id === 'nangele' || id === 'original-tracks') {
                let htmlStr = '';
                
                if (id === 'gobunny') {
                    const hero = findFile('GOBUNNY MOCKUP');
                    const identity = findFiles('1.png').concat(findFiles('2.png'));
                    const product = findFiles('3.png').concat(findFiles('4.png')).concat(findFiles('7.png'));
                    const digital = findFiles('5.png');
                    const merch = findFiles('6.png');
                    ...
                    htmlStr += `<div class="cs-section"><h2 class="cs-heading">Hero Image</h2><div class="masonry-container">`;
                    if(hero) htmlStr += renderMedia(hero).replace('masonry-item', 'masonry-item full-width');
                    htmlStr += `</div></div>`;

                    htmlStr += `<div class="cs-section"><h2 class="cs-heading">Identity & Guidelines</h2><div class="masonry-container">`;
                    identity.forEach(f => { htmlStr += renderMedia(f); });
                    htmlStr += `</div></div>`;

                    htmlStr += `<div class="cs-section"><h2 class="cs-heading">Product & Packaging</h2><div class="masonry-container">`;
                    product.forEach(f => { htmlStr += renderMedia(f); });
                    htmlStr += `</div></div>`;

                    htmlStr += `<div class="cs-section"><h2 class="cs-heading">Digital & Web</h2><div class="masonry-container">`;
                    digital.forEach(f => { htmlStr += renderMedia(f).replace('masonry-item', 'masonry-item full-width'); });
                    htmlStr += `</div></div>`;
...
with open('project.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Applied all fixes perfectly")
