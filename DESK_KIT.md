...
**Phones (Sept 2026).** A phone (`(hover: none) and (pointer: coarse)`) starts in lite mode, does not draw the floating-doodle canvases (they only react to a mouse), does not draw the two full-screen blended layers (`html::after`, `body::after`), has no hero fade mask, and its peeking doodles use one soft shadow instead of five filters (section AN of the site CSS). `?lite=0` overrides. Sound: a phone will not start audio after a reload without a tap, so the engine waits for the first real tap (`touchend` / `pointerup` / `click` / `keydown`; a `pointerdown` does not count on an iPhone) and only stops waiting once the audio is running; meanwhile the speaker button pulses (`.is-armed`) so it is clear it is waiting. `js/room-sound.js` is `?v=2` on all three pages.
