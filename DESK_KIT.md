...
The first stamp pressed there earns one sticker, "Your page" (`hnRewards.notebook()`), and the book's counter becomes n / 40 (it was 38 before the photo and lamp stickers);
...
**The desk lamp is back, as an object (Sept 2026).** `initDeskLamp` puts a hand-drawn lamp on the hero desk (`button.hn-lamp` inside `#hero`, right-hand margin, hidden at 700px and below). Click switches it on and off; drag moves it (clamped inside the hero, kept clear of the hero's bottom edge so its pool is never cut); the head sways with the drag and settles with an overshoot. By default it follows day and night (unlit by day, lit at night); switching it by hand overrides that until the next theme switch. It is built to the rule above: it sets no custom property on the root, moves only with the `translate` property (0.07 ms per move measured), and its light is a plain translucent warm gradient (peak alpha .17, checked so ink stays above 4.5:1 at the brightest point), hidden in lite mode. Moving it once earns the eighth secret (`secret-lamp`, counter is now n / 40).
