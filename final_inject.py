...
start_str = "if (id === 'gobunny' || id === 'green-arrow') {"
end_str = "htmlStr += `</div></div>`;\n                }"
...
# 4. We must add the post-inject JS right after `gal.innerHTML = htmlStr;`
# But only if it's not already there.
post_inject = """                if (id === 'nangele') {
...
if "const renderBook" not in html:
    html = html.replace("gal.innerHTML = htmlStr;", "gal.innerHTML = htmlStr;\n" + post_inject)

with open('project.html', 'w', encoding='utf-8') as f:
    f.write(html)
print("Massive injection success!")
